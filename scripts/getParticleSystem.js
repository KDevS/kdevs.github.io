...
function getParticleSystem(params) {
  ...
  let { camera, emitter, parent, rate, texture, radius, maxLife, maxSize, maxVelocity, colorA, colorB, alphaMax } = params;
  const uniforms = {
    diffuseTexture: {
...
//   const maxSize = 0.5;
  ...
  const initSize = maxSize;
  const initLimit = 5 * maxSize;
  
  function _UpdateIntensity(delta)
  {
	  if ((maxSize < initSize) || (maxSize > initLimit)) return;
  ...
  function update(timeElapsed, rateUpdated, delta = 0) {
	_UpdateIntensity(delta);
    _AddParticles(timeElapsed, rateUpdated);
    _UpdateParticles(timeElapsed);
    _UpdateGeometry();
  }
